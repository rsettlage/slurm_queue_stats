...
## reservations
all_reservation <- fread("cat ./ca_reserved.txt ./dt_reserved.txt ./hu_reserved.txt",data.table=FALSE)
...
for(j in 1:nrow(all_queues)){
    for(i in 2:ncol(all_queues)){
        if(!is.na(all_queues[j,i])){
            if(grepl("-",all_queues[j,i])){
                temp <- separate(all_queues[j,c(1,i)],2,sep="-",c("start","end"),fill="right")
                all_queues[j,i] <- paste(all_queues$machine[j],sprintf("%03d",temp$start:temp$end),sep="",collapse=",")
            }
        }
    }
    temp <- gsub(",NA","",paste(all_queues[j,-1],collapse=","))
    all_queues[j,2] <- temp
}
...
for(j in 1:nrow(all_reservation)){
    if(!is.na(all_reservation[j,2])){
            #         DEQ                     077-084
            # reprovision 023,029,062,072-073,121,170
            temp1 <- unlist(strsplit(all_reservation[j,2],","))
            temp_nodelist <- c()
            for(i in 1:length(temp1)){
                temp2 <- unlist(strsplit(temp1[i],"-"))
                if(length(temp2)==2){
                    temp_nodelist <- c(temp_nodelist,paste(all_reservation$machine[j],sprintf("%03d",temp2[1]:temp2[2]),sep="",collapse=","))
                }else{
                    temp_nodelist <- c(temp_nodelist,paste0(all_reservation$machine[j],temp2[1]))
                }
            }
            all_reservation[j,2] <- paste(temp_nodelist,collapse=",")
    }
}
